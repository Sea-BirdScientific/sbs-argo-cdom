...
def simple_map(longitudes, latitudes, z=None, polygon=None, title='', fig=None, figIndex=None, marker=None, secondaries=None, bounding_box=None, s=100):
    fontsz = 20
    if fig:
    ...
    if z is not None:
        ...
        print('here')
        s = sns.scatterplot(ax=ax, x=longitudes, y=latitudes, hue=z, transform=ccrs.PlateCarree(), s=s)
        # plt.colorbar(s, pad=0.1)
    else:
        s = ax.scatter(longitudes, latitudes,transform=ccrs.PlateCarree(), s=s)
    ...
    if secondaries:
        for sec in secondaries:
            ax.scatter(sec['lon'], sec['lat'],transform=ccrs.PlateCarree(), color='red', marker='x', s=s)
